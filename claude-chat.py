                        ...
                        if isinstance(content, str):
                            ...
                            for block in content:
                                ...
                                if btype == "text":
                                    ...
                                    tc = ToolCall(block.get("name", "unknown"), block.get("input", {}))
...
def _auto_link_urls(text):
    ...
    return re.sub(
        r'(?<!href=")(?<!src=")(https?://[^\s<>")\]]+)', r'<a href="\1" target="_blank" rel="noopener">\1</a>', text
    )
    ...
    for m in session.messages:
        ...
        if m.text:
            ...
            if rich:
                ...
                text = re.sub(
                    r"^(#{1,6})\s+(.+)$",
                    lambda m: f'<h{len(m.group(1))} class="md-heading">{m.group(2)}</h{len(m.group(1))}>',
                    text,
                    flags=re.MULTILINE,
                )
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    result = (
        HTML_TEMPLATE.replace("{{MESSAGES}}", "\n".join(messages_html))
...
{{MESSAGES}}
<div class="footer">Exported with claude-chat v"""
    + __version__
    + """</div>
{{RICH_FOOT}}
</body>
