...
result: issue
reported: "TypeError: unsupported operand type(s) for |: 'type' and 'NoneType' at sync_chats.py:190 (`def load_config() -> dict | None:`). Script never reaches cmd_mine — module import itself fails under macOS system python3 (3.9.x) because PEP 604 union syntax requires Python 3.10+."
severity: blocker
root_cause_hypothesis: "PATH scrub `/usr/bin:/bin` hides pipx/Homebrew/pyenv python3.14 and routes `python3` to macOS system 3.9, which cannot parse PEP 604 `X | Y` type hints at module load time."
fix_options:

- "Portability fix: add `from __future__ import annotations` at top of sync_chats.py so all type hints are stringified and bypass parse-time evaluation (single-line, zero runtime change, supports 3.7+)."
- "Alternate fix: rewrite PEP 604 annotations (`dict | None`, `list | None`, etc.) as `Optional[dict]` + `from typing import Optional`."
- "Procedure fix: update SKILL.md / SUMMARY human-check wording to invoke an explicit interpreter, e.g. `env PATH=/usr/bin:/bin /opt/homebrew/bin/python3 ~/.claude-chat/sync_chats.py mine` — keeps Python while hiding `mempalace`."
